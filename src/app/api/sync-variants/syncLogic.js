...
export async function startVariantsBackgroundSync(initialSkip = 0, targetShopifyId = null) {
  const client = await clientPromise;
  const db = client.db("next_local_db");

  // Only run if not already running for the specific type
  const activeSession = await db.collection("sync_sessions").findOne({ type: "variants", status: "running" });
  if (activeSession) {
    return { error: "A variants sync session is already running" };
  }
  ...
  await db.collection("sync_sessions").insertOne({
    ...
    status: "running",
    startedAt: new Date(),
    message: initialSkip > 0 ? `Resuming variant sync from ${initialSkip}...` : "Starting variant sync...",
    progress: 0,
    skip: initialSkip
  });
  ...
  (async () => {
    try {
      ...
      let productsToSync = [];
      if (targetShopifyId) {
        const product = await productsCollection.findOne({ shopifyId: targetShopifyId });
      ...
      if (productsToSync.length === 0) {
        await db.collection("sync_sessions").updateOne(
          { sessionId },
          { $set: { status: "completed", message: "No products found to sync variants.", progress: 100, endedAt: new Date() } }
        );
        return;
      }
      ...
      while (currentSkip < productsToSync.length) {
        try {
          const p = productsToSync[currentSkip];
          ...
          await db.collection("sync_sessions").updateOne(
            ...
            { 
              $set: { 
                ...
                progress: Math.round((currentSkip / productsToSync.length) * 100),
                skip: currentSkip
              } 
            }
          );
          ...
          if (currentSkip % 5 === 0) await sleep(500);

        } catch (fetchErr) {
          console.error(`Error syncing variant at index ${currentSkip}, retrying...`, fetchErr);
          await sleep(5000);
        }
      }
      ...
      await db.collection("sync_sessions").updateOne(
        ...
        { 
          $set: { 
            status: "completed", 
            message: `Sync Complete for ${productsToSync.length} products!`, 
            progress: 100,
            endedAt: new Date()
          } 
        }
      ...
      console.error("Critical Variant Sync Error:", error);
      await db.collection("sync_sessions").updateOne(
        { sessionId },
        { 
          $set: { 
  ...
  return { success: true, message: "Variant sync started in background" };
}