        ...
        <div className="group/card block space-y-4">
          <div className="relative aspect-square w-full bg-[#fafafa] overflow-hidden">
            <Link href={`/products/${product.handle}`} className="block w-full h-full mix-blend-multiply" onClick={handleProductClick}>
              {galleryImages.length > 0 ? (
                ...
                >
                  ...
                  {galleryImages.length > 1 && <div className={`pagination-${swiperId} swiper-pagination bottom-0! lg:!hidden`} />}
                </Swiper>
            ...
            {galleryImages.length > 1 && (
              <>
                <button ref={prevImageBtnRef} type="button" className={`custom-prev-${swiperId} absolute left-3 top-1/2 -translate-y-1/2 z-10 w-9 h-9 flex items-center justify-center rounded-full bg-white/85 text-black shadow-md opacity-0 group-hover/card:opacity-100 transition-opacity`}><ChevronLeft size={18} /></button>
                <button ref={nextImageBtnRef} type="button" className={`custom-next-${swiperId} absolute right-3 top-1/2 -translate-y-1/2 z-10 w-9 h-9 flex items-center justify-center rounded-full bg-white/85 text-black shadow-md opacity-0 group-hover/card:opacity-100 transition-opacity`}><ChevronRight size={18} /></button>
              </>
            )}
          </div>
