...
export default function FAQSection({ cityName, stateName, todayRate, sectionData }) {
  ...
  return (
    <section className="py-24 md:py-40 bg-[#FAF3EC]/30">
        ...
        <div className="space-y-6">
          {faqBlocks.map((block, index) => (
            ...
            >
              ...
              >
                <span className={`text-[14px] md:text-[18px] font-bold uppercase tracking-tight font-figtree transition-colors duration-300 ${
                    openIndex === index ? 'text-primary' : 'text-zinc-800'
                }`}>
                  {replaceShortcodes(block.settings.faq_question)}
                </span>
                <div className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center transition-colors ${
                    openIndex === index ? 'bg-primary text-white' : 'bg-zinc-50 text-zinc-400'
                }`}>
                    {openIndex === index ? (
                        <Minus size={16} />
                    ) : (
              ...
              >
                <div className="p-8 md:p-10 pt-0 text-zinc-500 leading-relaxed text-[14px] md:text-[18px] font-figtree border-t border-zinc-50 pt-8">
