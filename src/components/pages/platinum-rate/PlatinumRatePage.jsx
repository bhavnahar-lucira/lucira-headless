...
export default function PlatinumRatePage({ page }) {
    ...
    useEffect(() => {
        ...
        async function fetchRates() {
            try {
                const res = await fetch("/api/platinum-rates");
                if (res.ok) {
    ...
    const platinumWidgetSettings = useMemo(() => {
        ...
        return {
            ...base,
            rate_today: `₹ ${(Number(rates.platinum_price_999) || parseInt(base.rate_today.replace(/[^\d]/g, ''))).toLocaleString('en-IN')}`,
            rate_avg: `₹ ${(Number(rates.platinum_price_avg) || parseInt(base.rate_avg.replace(/[^\d]/g, ''))).toLocaleString('en-IN')}`,
        };
    }, [rates]);
                                        ...
                                        {platinumWidgetSettings.flip_founder_image && (
                                            <img
                                                src={platinumWidgetSettings.flip_founder_image.startsWith('shopify://')
                                                    ? platinumWidgetSettings.flip_founder_image.replace('shopify://shop_images/', 'https://www.lucirajewelry.com/cdn/shop/files/')
                                                    : platinumWidgetSettings.flip_founder_image
                                                }
